...
func TestRegistryAdministration_VendorClaim(t *testing.T) {
	...
	t.Run("ok - keys generated by crypto", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		_, err := cxt.registry.RegisterVendor("vendorId", "Test Vendor", domain.HealthcareDomain)
		if err != nil {
			panic(err)
		}
	...
	t.Run("ok - existing org keys in crypto", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		vendor, _ := cxt.registry.RegisterVendor("vendorId", "Test Vendor", domain.HealthcareDomain)
