...
func TestRegistryAdministration_RegisterEndpoint(t *testing.T) {
	...
	t.Run("ok", func(t *testing.T) {
		...
		_, err := cxt.registry.RegisterVendor("urn:oid:1.3.6.1.4.1.54851.4:4", "vendor", domain.HealthcareDomain)
		if !assert.NoError(t, err) {
			return
		}
		_, err = cxt.registry.VendorClaim("urn:oid:1.3.6.1.4.1.54851.4:4", "orgId", "org", nil)
		if !assert.NoError(t, err) {
			return
		}
	...
	t.Run("ok - auto generate id", func(t *testing.T) {
		...
		cxt.registry.RegisterVendor("urn:oid:1.3.6.1.4.1.54851.4:4", "vendor", domain.HealthcareDomain)
		cxt.registry.VendorClaim("urn:oid:1.3.6.1.4.1.54851.4:4", "orgId", "org", nil)
	...
	t.Run("ok - org has no certificates", func(t *testing.T) {
		...
		cxt.registry.EventSystem.ProcessEvent(events.CreateEvent(domain.RegisterVendor, domain.RegisterVendorEvent{
			Identifier: "urn:oid:1.3.6.1.4.1.54851.4:4",
			Name:       "Test Vendor",
		}))
		cxt.registry.VendorClaim("urn:oid:1.3.6.1.4.1.54851.4:4", "orgId", "org", nil)
		event, err := cxt.registry.RegisterEndpoint("orgId", "", "url", "type", "status", map[string]string{"foo": "bar"})
...
func TestRegistryAdministration_VendorClaim(t *testing.T) {
	...
	t.Run("ok - keys generated by crypto", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		_, err := cxt.registry.RegisterVendor("urn:oid:1.3.6.1.4.1.54851.4:4", "Test Vendor", domain.HealthcareDomain)
		if err != nil {
			panic(err)
		}
		registerEventHandler(cxt.registry)

		event, err := cxt.registry.VendorClaim("urn:oid:1.3.6.1.4.1.54851.4:4", t.Name(), "orgName", nil)
		if !assert.NoError(t, err) {
			return
		}
		assert.NotNil(t, event.Signature())
		assert.Equal(t, "urn:oid:1.3.6.1.4.1.54851.4:4", string(payload.VendorIdentifier))
	...
	t.Run("ok - existing org keys in crypto", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		vendor, _ := cxt.registry.RegisterVendor("urn:oid:1.3.6.1.4.1.54851.4:4", "Test Vendor", domain.HealthcareDomain)
		...
		privKey, _ := cxt.registry.crypto.GetOpaquePrivateKey(types.LegalEntity{URI: "urn:oid:1.3.6.1.4.1.54851.4:4"})
		...
		event, err := cxt.registry.VendorClaim("urn:oid:1.3.6.1.4.1.54851.4:4", org.URI, orgName, []interface{}{jwkAsMap})
		if !assert.NoError(t, err) {
			return
		}
		assert.NotNil(t, event.Signature())
	...
	t.Run("ok - vendor has no active certificates", func(t *testing.T) {
		...
		event, err := cxt.registry.VendorClaim("urn:oid:1.3.6.1.4.1.54851.4:4", org.URI, "orgName", nil)
	...
	t.Run("error - vendor has no keys", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		cxt.registry.RegisterVendor("vendorWithoutCertificate", "Test Vendor Without Certificate", domain.HealthcareDomain)
		cxt.empty()
	...
	t.Run("error - unable to load existing key", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		cxt.registry.RegisterVendor("vendorID", "Test Vendor", domain.HealthcareDomain)
...
func TestRegistry_signAsOrganization(t *testing.T) {
	...
	t.Run("error - unable to sign JWS (vendor not registered)", func(t *testing.T) {
		cxt := createTestContext(t)
		defer cxt.close()
		_, err := cxt.registry.signAsOrganization("orgId", "Foobar", nil, time.Now(), true)
		assert.Contains(t, err.Error(), "current vendor not registered: urn:oid:1.3.6.1.4.1.54851.4:4")
	})
}
