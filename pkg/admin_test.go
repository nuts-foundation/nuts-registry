...
func TestRegistryAdministration_VendorClaim(t *testing.T) {
	...
	t.Run("ok - existing org keys in crypto", func(t *testing.T) {
		...
		_, err = cxt.registry.VendorClaim(org.URI, orgName, []interface{}{jwkAsMap})
		if !assert.NoError(t, err) {
			return
		}
	})
	...
	t.Run("error - vendor has no active certificates", func(t *testing.T) {
		...
		_, err := cxt.registry.VendorClaim(org.URI, "orgName", nil)
		assert.Equal(t, err.Error(), "vendor has no active certificates (id = urn:oid:1.3.6.1.4.1.54851.4:4)")
	})
