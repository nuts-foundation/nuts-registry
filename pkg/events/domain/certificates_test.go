...
func Test_CertificateEventHandler_HandleEvent(t *testing.T) {
	...
	t.Run("error - vendor claim - organization certificate not trusted", func(t *testing.T) {
		...
		err := handler.handleEvent(events.CreateEvent(VendorClaim, event, nil), nil)
		assert.EqualError(t, err, "certificate problem in VendorClaim event: certificate not trusted: SERIALNUMBER=07969287,CN=Go Daddy Secure Certification Authority,OU=http://certificates.godaddy.com/repository,O=GoDaddy.com\\, Inc.,L=Scottsdale,ST=Arizona,C=US (issuer: OU=Go Daddy Class 2 Certification Authority,O=The Go Daddy Group\\, Inc.,C=US, serial: 769): x509: certificate signed by unknown authority")
